...
int Skiplist<K, V>::InsertNode(K key, V value) {
  ...
  if (randLevel > _listLevel) {
    for (int i = _listLevel + 1; i <= randLevel; i++) update[i] = _header;
...
void Skiplist<K, V>::DeleteNode(K key) {
  ...
  if (current && current->GetKey() == key) {
    // 从高层向下逐层删除节点
    for (int i = _listLevel; i >= 0; i--) {
      if (update[i]->_forward[i] != current) continue;
      update[i]->_forward[i] = std::move(current->_forward[i]);
    }
    // 调整跳表的当前高度
    while (_listLevel > 0 && !_header->_forward[_listLevel]) _listLevel--;
...
template <typename K, typename V>
void Skiplist<K, V>::GetKVfromStr(const std::string &str, K &key, V &value,
                                  int &valid) {
  if (!IsValidKV(str)) return;
